...
def test_cache_count(edge_node):
    result = edge_node.cache_file("sample.py", "This is a sample python file")
    value = edge_node.get_pending_sync_count()
    assert value == 1

def test_sync_to_cloud(cloud_node, edge_node):
    main = edge_node.cache_file("Difficult_file.apk", "Should see whether it works or not")
...
def test_read_storage(storage_manager):
    result = storage_manager.write_file("test.txt", "Hello file")
